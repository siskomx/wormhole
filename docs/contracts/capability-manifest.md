...
- `orchestration.repo-reachability-review`: read-only repo-wide reachability evidence collection for coding-agent deletion review with explicit entrypoints, package boundaries, conservative dynamic/framework handling, optional Knip hints, review categories, and mandatory human approval for removals.
- `orchestration.repo-native-coverage-pack`: read-only repo-native coverage pack and feature-slice query layer built on the existing feature index, project contract, repo index, source-conflict analysis, test impact, verification planner, repo-local scripts, conventions, schema evidence, and relation audit.
